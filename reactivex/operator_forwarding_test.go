...
func TestAdapter_OnErrorCompleteForwarding(t *testing.T) {
	...
	t.Run("Map", func(t *testing.T) {
		...
		s.OnError(want)
		if !errors.Is(capt.err, want) {
			t.Fatalf("OnError not forwarded: got %v", capt.err)
		}
		capt.err = nil
		s.OnComplete()
		if !capt.done {
	...
	t.Run("Take_ForwardsErrorWhenLeftPositive", func(t *testing.T) {
		capt := &captureSubscriber[int]{}
		s := &takeSubscriber[int]{out: capt, sub: nullSubscription{}, left: 5}
		s.OnError(want)
		if !errors.Is(capt.err, want) {
			t.Fatalf("OnError not forwarded: got %v", capt.err)
		}
	...
	t.Run("Take_DropsErrorWhenLeftZero", func(t *testing.T) {
		...
		capt := &captureSubscriber[int]{}
		s := &takeSubscriber[int]{out: capt, sub: nullSubscription{}, left: 0}
		s.OnError(want)
		if capt.err != nil {
			t.Fatalf("expected error to be dropped, got %v", capt.err)
		}
	})
	...
	t.Run("Take_OnCompleteForwardedWhenLeftPositive", func(t *testing.T) {
		capt := &captureSubscriber[int]{}
		s := &takeSubscriber[int]{out: capt, sub: nullSubscription{}, left: 2}
		s.OnComplete()
		if !capt.done {
	...
	t.Run("Skip", func(t *testing.T) {
		capt := &captureSubscriber[int]{}
		s := &skipSubscriber[int]{out: capt, sub: nullSubscription{}, left: 0}
		s.OnError(want)
		if !errors.Is(capt.err, want) {
			t.Fatalf("OnError not forwarded: got %v", capt.err)
		}
	...
	t.Run("Reduce_OnCompleteEmitsPartial", func(t *testing.T) {
		...
		capt := &captureSubscriber[int]{}
		s := &reduceSubscriber[int]{
			out: capt, f: func(a, b int) int { return a + b },
		}
		s.OnNext(7)
		s.OnComplete()
		if !capt.done {
		...
		if capt.count != 1 {
			t.Fatalf("Reduce should have emitted the accumulator once; got %d", capt.count)
		}
	})
	...
	t.Run("Reduce_OnCompleteEmptyInput", func(t *testing.T) {
		...
		if capt.count != 0 {
			t.Fatalf("Reduce must not emit a value on empty input; got %d", capt.count)
		}
	})
}
