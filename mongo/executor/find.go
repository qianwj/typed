...
type FindExecutor[D bson.Doc[I], I bson.ID] struct {
	...
	oneOpts         *rawopts.FindOneOptions
}
...
func (f *FindExecutor[D, I]) AllowPartialResults() *FindExecutor[D, I] {
	...
	f.oneOpts.SetAllowPartialResults(true)
	return f
}
...
func (f *FindExecutor[D, I]) Collation(collation *options.Collation) *FindExecutor[D, I] {
	...
	f.oneOpts.SetCollation((*rawopts.Collation)(collation))
	return f
}
...
func (f *FindExecutor[D, I]) NoCursorTimeout() *FindExecutor[D, I] {
	...
	f.oneOpts.SetNoCursorTimeout(true)
	return f
}
...
func (f *FindExecutor[D, I]) Snapshot() *FindExecutor[D, I] {
	...
	f.oneOpts.SetSnapshot(true)
	return f
}

func (f *FindExecutor[D, I]) One(ctx context.Context) (D, error) {
	var (
		data D
		res  *raw.SingleResult
	)
	if f.primary {
		res = f.readprefPrimary.FindOne(ctx, f.filter, f.oneOpts)
	} else {
		res = f.readprefDefault.FindOne(ctx, f.filter, f.oneOpts)
	}
	if res.Err() != nil {
		return data, res.Err()
	}
...
func (f *FindExecutor[D, I]) CollectFirst(ctx context.Context, data any) error {
