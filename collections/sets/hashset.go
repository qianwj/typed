...
// option.Option[T], or an absent Option when the set is empty.
//
...
func (s *HashSet[T]) MinBy(less func(x, y T) int) option.Option[T] {
	var (
		best  T
		found bool
	)
...
func (s *HashSet[T]) MaxBy(less func(x, y T) int) option.Option[T] {
	var (
		best  T
		found bool
	)
...
func (s *HashSet[T]) Find(p func(T) bool) option.Option[T] {
	for _, value := range s.Collect() {
		if p(value) {
