...
Calls `f` exactly `times` times and discards the adt. `times <= 0` is a no-op (the body is never entered and `f` is not called). It is equivalent to `for i := 0; i < times; i++ { f() }`, just more explicit.
